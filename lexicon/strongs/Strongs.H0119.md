**<big><span class="ashebrew">‎אָדַם‎</span></big>** **ʼâdam** <small>*(aw-dam')*</small> ==**to redden**==  <sup><small>[[TEHMC#H:V|H:V]]</small></sup> 
<small>to show blood (in the face), i.e.</small>

$\quad$ be (dyed, made) red (ruddy).
- ***flush*** or ***turn rosy***

**1)** to be red, red
$\quad$ **1a)** (Qal) ruddy (of Nazarites)
$\quad$ **1b)** (Pual)
$\quad$ $\quad$ **1b1)** to be rubbed red
$\quad$ $\quad$ **1b2)** dyed red
$\quad$ $\quad$ **1b3)** reddened
$\quad$ **1c)** (Hiphil)
$\quad$ $\quad$ **1c1)** to cause to show red
$\quad$ $\quad$ **1c2)** to glare
$\quad$ $\quad$ **1c3)** to emit (show) redness
$\quad$ **1d)** (Hithpael)
$\quad$ $\quad$ **1d1)** to redden
$\quad$ $\quad$ **1d2)** to grow red
$\quad$ $\quad$ **1d3)** to look red

<small>See Greek: </small>**<big>[[G4448|πυρόω]]</big>**
